...
filename = strcat({'../../../output/mouse/figures/graph_data_diffusion_regularization.png'});
...
filename = strcat({'../../../output/mouse/figures/zscore_MSEvst.png'});
saveas(gcf, char(filename))

figure()
plot(MSE_tr,'LineWidth',2); hold on
plot(MSE_ts,'LineWidth',2)
xline(5,'--r','LineWidth',1,'Label','Diffusion time 5','FontSize',15,'Color','k')
legend('Training','Test')
set(gca,'FontSize',15)

filename = strcat({'../../../output/mouse/figures/MSEvst.png'});
saveas(gcf, char(filename))

%% Regularizing activities using optimum t = 5 

Areg = M^5 * Atr'; % [cellsxfactors]

figure()
histogram(Areg,181);
set(gca,'FontSize',15)

filename = strcat({'../../../output/mouse/figures/tf_activities_distribution.png'});
saveas(gcf, char(filename))
...
writetable(Areg2export,'../../../output/mouse/regularized_activities.csv',...
